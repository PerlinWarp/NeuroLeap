	...
	if (points is not None):
		...
		for i in range(0,5):
			...
			if (i == 0):
				...
				m_p = dot_product_angle(metacarpal_xy, proximital_xy)
				# Intermediate Proximital
				p_i = dot_product_angle(proximital_xy, intermediate_xy)
				# Distal Intermediate Angle
				i_d = dot_product_angle(intermediate_xy, distal_xy)

				print("Thumb Angles Above: ",round(m_p,2), round(p_i,2), round(i_d,2))
				...
				m_p = dot_product_angle(metacarpal_xz, proximital_xz)
				# Intermediate Proximital
				p_i = dot_product_angle(proximital_xz, intermediate_xz)
				# Distal Intermediate Angle
				i_d = dot_product_angle(intermediate_xz, distal_xz)

				print("Thumb Angles Behind: ",round(m_p,2), round(p_i,2), round(i_d,2))
				...
				m_p = dot_product_angle(metacarpal_yz, proximital_yz)
				# Intermediate Proximital
				p_i = dot_product_angle(proximital_yz, intermediate_yz)
				# Distal Intermediate Angle
				i_d = dot_product_angle(intermediate_yz, distal_yz)

				print("Thumb Angles Side: ",round(m_p,2), round(p_i,2), round(i_d,2))
